...
SELECT EMP_NAME AS "사원 명", SALARY*12 AS "연봉", (SALARY + SALARY* BONUS)*12 AS "총수령액", ((SALARY + SALARY* BONUS)*12)-((SALARY*12)*0.3) AS "실수령액"
FROM EMPLOYEE
WHERE BONUS IS NOT NULL;
--7. EMPLOYEE테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급, 고용일, 연락처 조회
SELECT EMP_NAME AS "사원명" , SALARY , HIRE_DATE, PHONE, SAL_LEVEL
FROM EMPLOYEE
WHERE SAL_LEVEL = 'S1';
--8. EMPLOYEE테이블에서 실수령액(6번 참고)이 5천만원 이상인 사원의 이름, 월급, 실수령액, 고용일 조회
SELECT EMP_NAME AS "사원명", SALARY,((SALARY + SALARY* BONUS)*12)-((SALARY*12)*0.3) AS "실수령액", HIRE_DATE AS "고용일"
FROM EMPLOYEE   --오류
WHERE BONUS IS NOT NULL;
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND HIRE_DATE>'02/01/01';
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '____%' AND DEPT_CODE IN('D9','D6') AND HIRE_DATE BETWEEN '90/01/01' AND '00/12/01' AND SALARY>=2700000;
--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT SUBSTR(EMP_NO,1,2)||'년'AS"생년", SUBSTR(EMP_NO,3,2)||'월'AS"생월",SUBSTR(EMP_NO,5,2)||'일'AS"생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME AS"사원명",SYSDATE AS"입사일", FLOOR(SYSDATE - HIRE_DATE) AS "근무일수1"
FROM EMPLOYEE; 
...
SUBSTR(EMP_NO,1,2)||'년'AS"생년",
SUBSTR(EMP_NO,3,2)||'월'AS"생월",
SUBSTR(EMP_NO,5,2)||'일'AS"생일", 
TO_CHAR(-SUBSTR(EMP_NO,1,2),'YY')||'살'AS"나이(만)"
FROM EMPLOYEE;
...
SELECT EMP_NAME AS "사원명", SUBSTR(EMP_NO,1,6)AS"주민번호 앞자리" , SUBSTR(EMP_NO,8,14)AS"주민번호 뒷자리",SUBSTR(EMP_NO,1,6)+SUBSTR(EMP_NO,8,14)AS"주민번호 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;

--24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 
SELECT EMP_NAME AS "사원 명" ,DEPT_CODE AS "부서코드", (SALARY+SALARY*NVL(BONUS,0))*12 AS "보너스포함 연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
SELECT 
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),1998,1)),0) AS "1998년",
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),1999,1)),0) AS "1999년",
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),2000,1)),0) AS "2000년",
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),2001,1)),0) AS "2001년",
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),2002,1)),0) AS "2002년",
NVL(SUM(DECODE( EXTRACT(YEAR FROM HIRE_DATE),2003,1)),0) AS "2003년",
