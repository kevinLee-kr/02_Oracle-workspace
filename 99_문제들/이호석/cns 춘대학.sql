...
AS SELECT STUDENT_NO, STUDENT_NAME, STUDENT_ADDRESS, ABSENCE_YN
    FROM TB_STUDENT;
--오류 발생, CRAETE 권한이 없기때문임. 권한 부여를 해주자.
GRANT CREATE VIEW TO CNS; -- 관리자 계정에서 실행
...
AS SELECT S.STUDENT_NAME AS "학생이름", D.DEPARTMENT_NAME AS "학과이름", NVL(PROFESSOR_NAME, '교수 없음') AS "담당교수이름"
    FROM TB_STUDENT S
...
CREATE VIEW VW_STUDENT_P
AS SELECT DEPARTMENT_NAME, CAPACITY
    FROM TB_DEPARTMENT;
    
    SELECT DEPARTMENT_NAME, CAPACITY||'명'
    FROM VW_STUDENT_P;


--13. 위에서 생성한 학생일반정보 View를 통해서 학번이 A213046인 학생의
--이름을 본인 이름으로 변경하는 SQL 문을 작성하시오.
SELECT * FROM VW_STUDENT_INFO;

UPDATE VW_STUDENT_INFO
SET STUDENT_NAME = '이호석'
WHERE STUDENT_NO = 'A213046';

SELECT STUDENT_NAME, STUDENT_NO
FROM VW_STUDENT_INFO
WHERE STUDENT_NAME = '이호석';
